pipeline {
  agent any
  parameters {
    booleanParam(name: 'SKIP_CI', defaultValue: false, description: 'Skip CI stages if true')
    booleanParam(name: 'SKIP_CD', defaultValue: false, description: 'Skip CD stages if true')
  }
  environment {
    AWS_REGION = 'us-east-1'
    ECR_ACCOUNT = '242416990744'
    // repository name matches AWS ECR (no hyphen)
    ECR_REPO = 'patientservice'
